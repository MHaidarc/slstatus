...
static int load_ieee80211_nodereq(const char *interface,
...
int load_ieee80211req(int sock, const char *interface, void *data, int type,
                      size_t *len) {
  char warn_buf[256];
  ...
  if (ioctl(sock, SIOCG80211, &ireq) < 0) {
    snprintf(warn_buf, sizeof(warn_buf), "ioctl: 'SIOCG80211': %d", type);
    warn(warn_buf);
    return 0;
  }
...
const char *wifi_perc(const char *interface) {
  ...
  if (load_ieee80211req(sockfd, interface, &bssid, IEEE80211_IOC_BSSID, &len)) {
    ...
    len = sizeof(info);
    if (load_ieee80211req(sockfd, interface, &info, IEEE80211_IOC_STA_INFO,
                          &len)) {
      rssi_dbm = info.sta.info[0].isi_noise + info.sta.info[0].isi_rssi / 2;
